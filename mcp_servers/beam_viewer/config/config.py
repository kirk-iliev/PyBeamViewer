...
def _get_config_path() -> Path:
    ...
    Checks the ``BEAMVIEWER_CONFIG`` environment variable first; falls back
    to ``config.json`` in the same directory as this file.
    """
    env = os.environ.get("BEAMVIEWER_CONFIG", "").strip()
    if env:
        return Path(env)
