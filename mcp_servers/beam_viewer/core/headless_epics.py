"""
headless_epics.py — Qt-free EPICS / Channel Access communication layer.

Drop-in replacement for core/epics_layer.py that uses plain
``threading.Thread`` and Python callbacks instead of QThread / pyqtSignal.

Hybrid implementation:
1. Native Mode (Control Room): Uses caproto.threading.client.Context
...
OnNewFrame = Callable[[np.ndarray], None]
OnConnectionChanged = Callable[[bool], None]
OnError = Callable[[str], None]


# ---------------------------------------------------------------------------
# Producer thread  (Qt-free replacement for EpicsWorker)
# ---------------------------------------------------------------------------

class HeadlessEpicsWorker(threading.Thread):
    """EPICS image subscription worker using plain threads and callbacks.

    Drop-in replacement for the Qt-based ``EpicsWorker``.  Emits data via
    registered Python callables instead of ``pyqtSignal``.
    """
    ...
    def __init__(
        ...
        debug: bool = False,
        *,
        on_new_frame: Optional[OnNewFrame] = None,
        on_connection_changed: Optional[OnConnectionChanged] = None,
    ...
    def set_on_new_frame(self, cb: Optional[OnNewFrame]) -> None:
        self._on_new_frame = cb

    def set_on_connection_changed(self, cb: Optional[OnConnectionChanged]) -> None:
        self._on_connection_changed = cb

    def set_on_error(self, cb: Optional[OnError]) -> None:
        self._on_error = cb

    # -- callback dispatch helpers ---------------------------------------------

    def _emit_new_frame(self, frame: np.ndarray) -> None:
        cb = self._on_new_frame
        if cb is not None:
            try:
                cb(frame)
            except Exception:
    ...
    def _emit_connection_changed(self, connected: bool) -> None:
        cb = self._on_connection_changed
        if cb is not None:
            try:
                cb(connected)
            except Exception:
                log.exception("on_connection_changed callback error")
    ...
    def stop(self) -> None:
        ...
        if self._started.is_set():
            ...
            if self.is_alive():
                log.warning("HeadlessEpicsWorker did not finish within 5 s.")
    ...
    def _run_native_mode(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if self.width_pv:
                    pv_names.append(self.width_pv)
                if self.height_pv:
                    pv_names.append(self.height_pv)
                ...
                attempt = 0
                self._emit_connection_changed(True)
                ...
                if self.width_pv and self.width_pv in pv_map:
                    def on_width(sub, response):
                        ...
                        if response.data is not None:
                            self._width = int(response.data[0])
                ...
                if self.height_pv and self.height_pv in pv_map:
                    def on_height(sub, response):
                        if self._stop_event.is_set():
                            return
                        if response.data is not None:
                            self._height = int(response.data[0])
                ...
                self._emit_connection_changed(False)
                try:
                    ctx.disconnect()
                except Exception:
                    pass
