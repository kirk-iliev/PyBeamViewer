...
from ..schemas.camera import CameraInfo, ExposureSet, GainSet, PrefixSelect
...
def get_camera_info(bridge: HeadlessBridge = Depends(get_bridge)):
    """Get current camera configuration and PV names."""
    return bridge.get_camera_info()


@router.post("/prefix", response_model=AckResponse)
def select_prefix(body: PrefixSelect, bridge: HeadlessBridge = Depends(get_bridge)):
...
def set_exposure(body: ExposureSet, bridge: HeadlessBridge = Depends(get_bridge)):
...
def set_gain(body: GainSet, bridge: HeadlessBridge = Depends(get_bridge)):
    """Set the camera gain."""
