...
from api.schemas.display import ColormapSet, ThemeInfo, VALID_COLORMAPS

router = APIRouter(prefix="/display", tags=["Display"])


@router.get("/theme", response_model=ThemeInfo)
def get_theme(bridge: ApiBridge = Depends(get_bridge)):
    """Get the current UI theme."""
    return {"theme": bridge.get_theme()}


@router.post("/theme/toggle", response_model=AckResponse)
def toggle_theme(bridge: ApiBridge = Depends(get_bridge)):
...
@router.get("/colormap")
def get_colormap(bridge: ApiBridge = Depends(get_bridge)):
...
def set_colormap(body: ColormapSet, bridge: ApiBridge = Depends(get_bridge)):
