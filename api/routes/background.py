...
def get_background_status(bridge: ApiBridge = Depends(get_bridge)):
    """Get background subtraction status."""
    return bridge.get_background_status()


@router.post("/acquire", response_model=AckResponse)
def acquire_background(bridge: ApiBridge = Depends(get_bridge)):
...
def set_subtraction(body: BackgroundSubtractionSet, bridge: ApiBridge = Depends(get_bridge)):
...
def save_background(bridge: ApiBridge = Depends(get_bridge)):
...
def load_background(body: BackgroundLoadRequest, bridge: ApiBridge = Depends(get_bridge)):
...
def list_backgrounds(bridge: ApiBridge = Depends(get_bridge)):
